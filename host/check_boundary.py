"""Reproduce a camera-boundary loop; optionally ask real Jev to escape it.
...
def main():
    ...
    fixture = json.loads(Path("/fixtures/left-boundary.json").read_text())
    ...
    try:
        ...
        # Test setup only: retreat from the recorded approach to a pipe along the visible
        # ground, then accumulate blocked left commands at the camera edge.
        for _ in range(40):
            step(Action.LEFT)
            camera = int(env.unwrapped.ram[0x71A]) * 256 + int(env.unwrapped.ram[0x71C])
            if snapshot.x <= camera + 1 and snapshot.grounded:
                break
        start_x, start_y = snapshot.x, snapshot.y
        assert start_x <= camera + 8, "Fixture did not reach the camera boundary"
        for _ in range(8):
            step(Action.LEFT)
        print(json.dumps({"start_x": start_x, "camera_x": camera}), flush=True)
        if args.jev:
        ...
        for index in range(16):
            ...
            action = policy.choose(context, tuple(Action)).action if policy else Action.RIGHT_JUMP
            ...
            if snapshot.x > start_x + 48:
                break
        assert snapshot.x > start_x + 48, "Still trapped at the left camera boundary"
